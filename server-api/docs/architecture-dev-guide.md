...
- `/api` 的全局限流、指定接口限流、MySQL 规则加载和内存热刷新统一参考 `docs/api-rate-limit-architecture.md`。
...
项目最低 Go 版本为 1.27，JSON 编解码统一使用标准库 `encoding/json/v2`。禁止在业务代码、基础设施代码、测试和代码生成模板中新增 `encoding/json`（v1）引用。

- 使用 `json.Marshal`、`json.Unmarshal`、`json.MarshalWrite` 或 `json.UnmarshalRead` 完成常规 JSON 编解码；需要 token 级处理时使用 `encoding/json/jsontext`。
- v2 默认拒绝无效 UTF-8 和重复对象成员；这些错误必须返回或记录，禁止静默忽略。对外接口的字段名、字段位置和响应结构仍必须遵循既有接口契约。
- 接入第三方库时，库的内部 JSON 实现不作强制替换；项目自身跨边界（HTTP、缓存、导入导出）代码必须遵循本规范。
- 如确需兼容 v1 语义，必须在调用点显式配置 v2 兼容选项，并在代码注释中说明兼容对象和退出条件；不得恢复全局 v1 依赖。

### 2.8 Go 版本与新特性选型

项目应优先使用 `go.mod` 所声明的最新稳定 Go 版本及其标准库特性；升级 Go 版本是常规技术演进，不应因沿用历史写法而放弃已稳定的新能力。

- 新代码优先选择当前 Go 版本提供的标准库 API、语言特性和工具链检查；只有标准库能力不能满足需求时，才引入第三方依赖。
- 升级后，应主动评估能提升安全性、性能、可观测性或可维护性的特性，并优先在新增代码和独立基础设施中采用；例如 `encoding/json/v2`、`runtime/pprof` 的新增 profile、标准库 `uuid` 与新版 `go vet` 检查。
- 不为“使用新特性”而改变已锁定的外部接口契约。语义可能变化的迁移必须先补充兼容测试；性能优化必须通过基准测试或 pprof 证明收益。
- 实验性能力（如需 `GOEXPERIMENT` 才能启用的特性）不得默认进入生产构建。只有在明确的收益、回退方案和专项测试齐备时，才可试点。
- 新增或升级 Go 版本后，CI 和本地开发工具链必须同步到 `go.mod` 指定版本，并运行 `go test`、`go vet` 等适用检查。
