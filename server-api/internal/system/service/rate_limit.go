...
func (s *RateLimitRule) PageList(req *rateLimitDto.RateLimitRulePageReq) (rateLimitDto.RateLimitRulePageResult, error) {
	req.HTTPMethod = strings.ToUpper(strings.TrimSpace(req.HTTPMethod))
	rules, total, err := s.repo.PageList(req)
	if err != nil {
		return rateLimitDto.RateLimitRulePageResult{}, err
	}
	...
	return rateLimitDto.RateLimitRulePageResult{
